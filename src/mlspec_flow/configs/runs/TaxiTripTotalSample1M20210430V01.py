...
class TaxiTripTotalSample1M20210430V01Run(Run):
    ...
    query      = f"""SELECT 
        ...
        FROM (
            (select * from `bigquery-public-data.chicago_taxi_trips.taxi_trips` where EXTRACT(YEAR FROM trip_start_timestamp) < 2018 and {constraints} limit 1000000) UNION ALL
            ...
            (select * from `bigquery-public-data.chicago_taxi_trips.taxi_trips` where EXTRACT(YEAR FROM trip_start_timestamp) = 2019 and {constraints} limit 100000)  UNION ALL
            (select * from `bigquery-public-data.chicago_taxi_trips.taxi_trips` where EXTRACT(YEAR FROM trip_start_timestamp) > 2019 and {constraints} limit 100000) 
        )
    """
